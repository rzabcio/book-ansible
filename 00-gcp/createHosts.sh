...
if [ -n $ANSIBLE1_IP ]; then
	sed -i "s/ansible1 .*/ansible1 ansible_host=$ANSIBLE1_IP/" $HOSTS_FILE
fi
if [ -n $ANSIBLE2_IP ]; then
	sed -i "s/ansible2 .*/ansible2 ansible_host=$ANSIBLE2_IP/" $HOSTS_FILE
fi
if [ -n $ANSIBLE3_IP ]; then
	sed -i "s/ansible3 .*/ansible3 ansible_host=$ANSIBLE3_IP/" $HOSTS_FILE
fi
